...
DY50_AckCode_t DY50_SendCommand(DY50_Typedef_t *dy50, uint8_t cmd, uint16_t payload_len)
{
	...
	uint8_t packet_length = code_len + payload_len + checksum_len;

	dy50->buf_tx.packet.length  = SWAP16(packet_length);

	DY50_CalcCheckSum(&dy50->buf_tx.packet, payload_len);
	...
	if(HAL_UART_Receive(dy50->huart, dy50->buf_rx.raw, ACK_PACKET_SIZE, 500) != HAL_OK)
...
DY50_AckCode_t DY50_CMD_VerifyPassword(DY50_Typedef_t *dy50, uint32_t password)
{
	...
	return DY50_SendCommand(dy50, DY50_CMD_VERIFY_PASSWORD, 4);
}
...
DY50_AckCode_t DY50_CMD_GetImage(DY50_Typedef_t *dy50)
{
	...
	return DY50_SendCommand(dy50, DY50_CMD_GET_IMAGE, PACKET_NOT_PAYLOAD);
}
...
DY50_AckCode_t DY50_CMD_GenChar(DY50_Typedef_t *dy50,  DY50_BufferId_t buffer_id)
{
	...
	return(DY50_SendCommand(dy50, DY50_CMD_GEN_CHAR, 1));
}
...
DY50_AckCode_t DY50_CMD_RegModel(DY50_Typedef_t *dy50)
{
	...
	return(DY50_SendCommand(dy50, DY50_CMD_REG_MODEL, PACKET_NOT_PAYLOAD));
}
