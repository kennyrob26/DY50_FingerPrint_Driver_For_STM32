...
static inline void DY50_BUILD_RegModel(DY50_Typedef_t *dy50)
{
	dy50->uart.tx_payload_len = PACKET_NOT_PAYLOAD;
	dy50->uart.rx_payload_len = PACKET_NOT_PAYLOAD;
}
...
/*
 * @brief Command RegModel (0x05) Sync version
 *
...
DY50_AckCode_t DY50_Async_CMD_RegModel(DY50_Typedef_t *dy50)
{
	...
	return(DY50_Async_SendCommand_Wait_Response(dy50, DY50_CMD_REG_MODEL, 500));
}
...
static inline void DY50_BUILD_StoreChar(DY50_Typedef_t *dy50, DY50_BufferId_t buffer_id, uint16_t page_id)
{
	dy50->uart.buf_tx.packet.payload[0] = buffer_id;
...
/*
 * @brief Command StoreChar (0x06) Sync version
 *
...
DY50_AckCode_t DY50_Sync_CMD_StoreChar(DY50_Typedef_t *dy50, DY50_BufferId_t buffer_id, uint16_t page_id)
{
	...
	DY50_BUILD_StoreChar(dy50, buffer_id, page_id);

	DY50_AckCode_t ack_code = DY50_Sync_SendCommand_Wait_Response(dy50, DY50_CMD_STORE_CHAR, 500);
...
/*
 * @brief Command StoreChar (0x06) Async version
 *
 * @note Stores the model generated by the RegModel command in the DY50 flash database
 *
 * @param buffer_id  the buffer where the template generated by RegModel is located (by default it is always in CharBuffer1)
...
static inline void DY50_BUILD_Search(DY50_Typedef_t *dy50, DY50_BufferId_t buffer_id, uint16_t start_page_id, uint16_t page_num)
{
	dy50->uart.buf_tx.packet.payload[0] = buffer_id;
...
/*
 * @brief Command Search (0x04) Async version
 *
...
DY50_AckCode_t DY50_Async_CMD_Search(DY50_Typedef_t *dy50, DY50_BufferId_t buffer_id, uint16_t start_page_id, uint16_t page_num)
{
	...
	DY50_BUILD_Search(dy50, buffer_id, start_page_id, page_num);

	return DY50_Async_SendCommand_Wait_Response(dy50, Dy50_CMD_SEARCH, 500);
}

/*
 * @brief Command Search (0x04) Sync version
 *
 * @note Search for a fingerprint in the DY50 flash database. Use this function to check if the fingerprint image
 *       we just read exists in the DY50 database.
 *
...
DY50_AckCode_t DY50_Sync_CMD_Search(DY50_Typedef_t *dy50, DY50_BufferId_t buffer_id, uint16_t start_page_id, uint16_t page_num)
{
	...
	return DY50_Sync_SendCommand_Wait_Response(dy50, Dy50_CMD_SEARCH, 500);
}
...
static inline void DY50_BUILD_DeletChar(DY50_Typedef_t *dy50, uint16_t start_id, uint16_t num_of_templates)
{
...
/*
 * @brief Command Delete Char (0x0C) Async version
 *
 * @note Deleting a set of IDs from the Flash database always requires a starting ID,
 *       and the number of IDs to be deleted starting from that ID will be specified
 *
 * @param dy50  Is a pointer for dy50 handler
 * @param start_id The first ID to be deleted
 * @param num_of_templates This is the number of IDs to be deleted (including the first one)
 *
...
DY50_AckCode_t DY50_Async_CMD_DeletChar(DY50_Typedef_t *dy50, uint16_t start_id, uint16_t num_of_templates)
{
	...
	DY50_BUILD_DeletChar(dy50, start_id, num_of_templates);

	return DY50_Async_SendCommand_Wait_Response(dy50, DY50_CMD_DELETE_CHAR, 500);
}

/*
 * @brief Command Delete Char (0x0C) Sync version
 *
 * @note Deleting a set of IDs from the Flash database always requires a starting ID,
 *       and the number of IDs to be deleted starting from that ID will be specified
 *
...
DY50_AckCode_t DY50_Sync_CMD_DeletChar(DY50_Typedef_t *dy50, uint16_t start_id, uint16_t num_of_templates)
{
	...
	return DY50_Sync_SendCommand_Wait_Response(dy50, DY50_CMD_DELETE_CHAR, 500);
}


static inline void DY50_BUILD_Empty(DY50_Typedef_t *dy50)
{
	dy50->uart.tx_payload_len = PACKET_NOT_PAYLOAD;
	dy50->uart.rx_payload_len = PACKET_NOT_PAYLOAD;
}
...
/*
 * @brief Command Empty (0x0D) Async Version
 *
 * @note Delete all models from the Flash memory
 *
 * @param dy50  Is a pointer for dy50 handler
 *
 * @returns the command state
